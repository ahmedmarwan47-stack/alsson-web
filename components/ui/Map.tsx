...
function assignMapRef(ref: ForwardedRef<MapLibreGL.Map>, value: MapLibreGL.Map | null) {
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => { assignMapRef(ref, mapInstance); return () => { assignMapRef(ref, null); }; }, [ref, mapInstance]);
...
function MarkerContent({ children, className }: MarkerContentProps) {
  ...
  return createPortal(
    <div className={cn("relative cursor-pointer", className)}>
      {normalizedChildren.length > 0 ? normalizedChildren : <div className="relative h-4 w-4 rounded-full border-2 border-white bg-blue-500 shadow-lg" />}
    </div>,
    marker.getElement(),
  );
}
...
function MapPopup({ longitude, latitude, onClose, children, className, closeButton = false, ...popupOptions }: MapPopupProps) {
  ...
  return createPortal(
    <div className={cn("bg-popover text-popover-foreground animate-in fade-in-0 zoom-in-95 relative rounded-md border p-3 shadow-md", className)}>
      {closeButton && (
        <button type="button" onClick={() => popup.remove()} className="ring-offset-background focus:ring-ring absolute top-1 right-1 z-10 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:ring-2 focus:ring-offset-2 focus:outline-none" aria-label="Close popup">
          <X className="h-4 w-4" />
        </button>
      )}
      {children}
...
function ControlButton({ onClick, label, children, disabled = false }: { onClick: () => void; label: string; children: React.ReactNode; disabled?: boolean }) {
  return (
    <button onClick={onClick} aria-label={label} type="button" className={cn("hover:bg-accent dark:hover:bg-accent/40 flex size-8 items-center justify-center transition-colors", disabled && "pointer-events-none cursor-not-allowed opacity-50")} disabled={disabled}>
      {children}
    </button>
  );
}
